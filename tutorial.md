...
There are a lot of builtin functions you can use while scripting:
...
Example script:
  ```
	...
	give_pocket(char, a) 
	moneytext = str(a)
	words = char + " opens the huge bag and finds "
	words = words + moneytext
	words = words + " dollars."
	reply(words)
  ```
This script picks a random number between 1 and 100.
It then gives the character using the script this amount of money.
(char is a predefined variable that references the character running the script)
Then the script creates a sentence and replies it in the chat.
The sentence will look similar to this: "Everythings opens the huge bag and finds 125 dollars."
...
if(a is 1){
give_character_item(char, "fractal_stone", 1)
reply("You found a fractal_stone.")}

if(a is 2){
give_character_item(char, "glow_stone", 1)
reply("You found a glow_stone.")}

if(a is 3){
give_character_item(char, "crystal_stone", 1)
reply("You found a crystal_stone.")}

if(a is 4){
give_character_item(char, "mosaic_stone", 1)
reply("You found a mosaic_stone.")}
  ```
This script randomly picks a number between 1 and 4, and depending on which number was picked gives the user a random item and replies which item the user got.
...
You can also create your own functions and use them in your script:
  ...
  myfunction()
  ```
  This code creates a function called myfunction() that replies with the text "Hello World" and then runs the function.
